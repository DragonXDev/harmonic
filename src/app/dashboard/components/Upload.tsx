...
const Upload: React.FC = () => {
  ...
  const handlePublish = () => {
    if (selectedFile) {
      ...
      // Here you can add logic to handle file upload or further processing
    } else {
      console.log('No file selected');
      // Trigger the file input dialog if no file is selected
  ...
  return (
    <Card className="grid h-full w-full grid-cols-1 gap-3 rounded-[20px] bg-white bg-clip-border p-3 font-dm shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:shadow-none 2xl:grid-cols-11">
      <div className="col-span-5 h-full w-full rounded-xl bg-lightPrimary dark:!bg-navy-700 2xl:col-span-6">
        <label htmlFor="file-upload" className="flex h-full w-full flex-col items-center justify-center rounded-xl border-[2px] border-dashed border-gray-200 py-3 dark:!border-navy-700 lg:pb-0">
          ...
          <input ref={fileInputRef} id="file-upload" type="file" accept=".mp3,.mp4,.mscz" style={{ display: 'none' }} onChange={handleFileChange} />
        </label>
      </div>
